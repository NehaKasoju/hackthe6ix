// NOTE: from QNX Open Source Gitlab. Here for reference.
// LINK: https://gitlab.com/qnx/sample-apps/camera_example1_callback/-/blob/main/camera_example1_callback.c?ref_type=heads
/*
...
static void processCameraData(camera_handle_t handle, camera_buffer_t* buffer, void* arg)
{
    ...
    case CAMERA_FRAMETYPE_RGB8888:
    {
        ...
        char filename[256];
        snprintf(filename, sizeof(filename), "frame_%04d.ppm", frameCount++);
        ...
        if (fp) {
            ...
            for (uint y = 0; y < height; y++) {
                ...
                for (uint x = 0; x < width; x++) {
                    uint8_t r = linePointer[x * 4 + 0];
                    uint8_t g = linePointer[x * 4 + 1];
                    uint8_t b = linePointer[x * 4 + 2];
                    fwrite(&r, 1, 1, fp);
                    fwrite(&g, 1, 1, fp);
                    fwrite(&b, 1, 1, fp);
                }
    ...
    case CAMERA_FRAMETYPE_BGR8888:
    {
        ...
        if (fp) {
            ...
            for (uint y = 0; y < height; y++) {
                uint8_t* linePointer = buffer->framebuf + y * stride;
                for (uint x = 0; x < width; x++) {
                    uint8_t b = linePointer[x * 4 + 0];
                    uint8_t g = linePointer[x * 4 + 1];
                    uint8_t r = linePointer[x * 4 + 2];
                    fwrite(&r, 1, 1, fp);
                    fwrite(&g, 1, 1, fp);
                    fwrite(&b, 1, 1, fp);
                }
